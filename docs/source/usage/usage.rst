...
possible rotations and translations of projections of the 3D structure
(templates).
...
- *Templates*, which are sets of 2D projections of a 3D structure or map at
  known viewing directions (poses), and
...
Template stacks used to running the likelihood
...
- Data that is specific to Templates (such as how the atomic
...
CryoLike's representations of images depend upon two grids:
one in two-dimensional Cartesian space, and one in Fourier
space with polar coordinates. These grids, along with a
precision (single or double), make up the required fields
...
not used when creating or interpreting Images.
...
Template conversion will place the generated files in a standard directory
...
and links to the relevant API calls.


Converting images
------------------------

CryoLike currently supports reading CryoSparc and Starfile image sources
and images stored in ``.mrc`` format. The conversion
process also handles restacking Images into standard sizes.

Converted Images stacks will be stored in a standard directory
structure. Each stack will have its own parameters file recording
