...
def fourier_polar_to_cartesian_phys(
    ...
    # if (grid_fourier_polar.x_points is None or grid_fourier_polar.y_points is None):
    #     raise ValueError("Polar grid does not have Cartesian-equivalent points set.")
    grid_fourier_polar.get_cartesian_points()
...
def volume_phys_to_fourier_points(
    ...
    if _device.type == 'cuda':
        ...
        fourier_slices = np.array(fourier_slices, dtype=np.float64)
