...
if TYPE_CHECKING: # pragma: no cover
    from cryolike.cross_correlation_likelihood import OptimalPoseReturn, integrated_log_likelihood_type, CrossCorrelationReturn
...
class LikelihoodOutputDataSources():
    ...
    ll_fourier_integrated: integrated_log_likelihood_type | None = None
...
class LikelihoodFileManager():
    ...
    def __init__(self,
        ...
        self._make_tree(folder_output, i_template)
        self._displacements_saved = False


    def _make_tree(self, folder_output: str, i_template: int):
    ...
    def _get_output_filenames(self, i_stack: int, types: OutputConfiguration) -> LikelihoodOutputFiles:
        ...
        if types.cross_correlation_pose:
            # cross correlation pose (the full smdw tensor) is mutually exclusive with all other
            # return types
    ...
    def load_template(self, params_input: str | ImageDescriptor, max_displacement_pixels: float, i_template: int):
        ...
        return (tp, image_desc, torch_float_type, max_displacement)
    ...
    def save_displacements(self, x_disp: Tensor, y_disp: Tensor):
        ...
        Args:
            x_disp (Tensor): Realized x-displacements on exponential scale
            y_disp (Tensor): Realized y-displacements on exponential scale
        """
        if self._displacements_saved:
            return
        
        displacements_filename = self._output_base / 'displacements_set.pt'
        displacements = stack([x_disp, y_disp], dim = 1).T.cpu().numpy()
    ...
    def write_outputs(self,
        ...
        if (outs.optimal_pose):
            ...
            save(out_data.optimal_pose.cross_correlation_S, out_fns.cross_corr_file)
            save(out_data.optimal_pose.optimal_template_S, out_fns.template_indices_file)
            save(out_data.optimal_pose.optimal_displacement_x_S, out_fns.x_displacement_file)
            save(out_data.optimal_pose.optimal_displacement_y_S, out_fns.y_displacement_file)
            save(out_data.optimal_pose.optimal_inplane_rotation_S, out_fns.inplane_rotation_file)
