...
from cryolike.util import Precision, set_precision, AtomicModel
...
from cryolike.microscopy import parse_parameters, save_parameters, print_parameters, ViewingAngles
    ...
    for i, input in enumerate(list_of_inputs):
        ...
        if isinstance(input, str):
            if input.endswith('.mrc') or input.endswith('.mrcs') or input.endswith('.map'):
                ...
                print("pdb_name:", name)
                # TODO: These should probably be actual checks
                # assert params.atom_radii is not None
                # assert isinstance(params.box_size, float)
            ...
            name = "tensor%d" % i
            density_physical_data = PhysicalVolume(density_physical=input, voxel_size=params.voxel_size)
        ...
        if flag_plots:
            ...
            plot_images(tp.templates_fourier, grid=tp.polar_grid, n_plots=16, filename=os.path.join(folder_output_plots, "templates_fourier_%s.png" % name), show=False)
            plot_power_spectrum(source=tp, filename_plot=os.path.join(folder_output_plots, "power_spectrum_%s.png" % name), show=False)
            phys_grid = CartesianGrid2D(n_pixels=params.n_voxels, pixel_size=params.voxel_size)
            templates_phys = tp.transform_to_spatial(phys_grid=phys_grid, n_templates_stop=16, use_cuda=use_cuda, save_to_class=False)
            plot_images(templates_phys, grid=phys_grid, n_plots=16, filename=os.path.join(folder_output_plots, "templates_phys_%s.png" % name), show=False)
