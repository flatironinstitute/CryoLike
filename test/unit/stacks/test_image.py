...
@patch(f"{PKG}.translation_kernel_fourier")
def test_displace_images_fourier(mock_tk: Mock):
    ...
    mock_tk.return_value = my_inv

    x_disp = torch.tensor([4.])
    y_disp = torch.tensor([6.])
    sut.displace_images_fourier(x_disp, y_disp)
    assert_close(sut.images_fourier, fimgs * my_inv)
    calls = mock_tk.call_args[0]
    assert calls[0] == fgrid
    assert_close(calls[1], x_disp[None,:])
    assert_close(calls[2], y_disp[None,:])
    assert calls[3] == Precision.DOUBLE


    sut = Images(fourier_data=make_mock_data_obj(fimgs.to(dtype=torch.complex64), fgrid),
                 box_size=np.array([4., 4.])
                 )
    x_disp2 = np.array([2., 4., 6., 8., 10.])
    y_disp2 = np.array([10., 8., 6., 4., 2.])
    sut.displace_images_fourier(x_disp2, y_disp2)
    assert_close(sut.images_fourier, fimgs * my_inv)
    calls = mock_tk.call_args[0]
    assert calls[0] == fgrid
    assert_close(calls[1], torch.tensor([1., 2., 3., 4., 5.], dtype=torch.float64))
    assert_close(calls[2], torch.tensor([5., 4., 3., 2., 1.], dtype=torch.float64))
    assert calls[3] == Precision.SINGLE


def test_displace_images_fourier_throws_on_bad_displacement_lengths():
    n_imgs = 5
    fimgs = make_image_tensor(n_imgs, 6, 6, target_fourier=True)
    fgrid = make_mock_polar_grid()
    img = Images(fourier_data=make_mock_data_obj(fimgs, fgrid))

    with raises(ValueError, match="must have the same length"):
        img.displace_images_fourier(x_displacements=np.array([1., 2.]), y_displacements=1., precision=Precision.DOUBLE)

    with raises(ValueError, match="must be 1 or equal to the number of images"):
        img.displace_images_fourier(x_displacements=np.array([1., 2.]), y_displacements=np.array([1., 2.]))
