...
def test_init_output_folders_skips_if_dry_run(tmp_path: Path):
    i_template = 12

    sut = _fix_get_mgr(tmp_path, 0, with_inputs_missing=True, i_template=i_template, dry_run=True)
    assert not sut._output_log_likelihood.is_dir()
    assert not sut._output_cross_correlation.is_dir()
    assert not sut._output_optimal_pose.is_dir()
...
def test_load_template(tmp_path, max_disp_p, pixel_size, i_template):
    ...
    with (
        ...
        res = sut.load_template(params_input, i_template)
...
def test_save_displacements(tmp_path, already_done):
    ...
    with (
        ...
        sut.save_displacements(disp_grid)
        if already_done:
    ...
    for outtype in ALL_OUTPUTS:
        ...
        with patch(f"{PKG}.save") as save:
            ...
            if outtype.optimal_pose:
                ...
                save.assert_any_call(out_data.optimal_pose.cross_correlation_M, expected_fns.cross_corr_file)
                save.assert_any_call(out_data.optimal_pose.optimal_template_M, expected_fns.template_indices_file)
                save.assert_any_call(out_data.optimal_pose.optimal_displacement_x_M, expected_fns.x_displacement_file)
                save.assert_any_call(out_data.optimal_pose.optimal_displacement_y_M, expected_fns.y_displacement_file)
                save.assert_any_call(out_data.optimal_pose.optimal_inplane_rotation_M, expected_fns.inplane_rotation_file)
